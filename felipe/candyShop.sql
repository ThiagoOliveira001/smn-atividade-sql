...
create table Administracao.empresa(
	...
	estado varchar(2) not null constraint chk_estado_empresa check ()
)
go
...
create table Administracao.responsavel(
	idfuncionario constraint pk_responsavel primary key identity(1,1),
...
create table Pessoa.telefone(
	...
	idfuncionario not null constraint fk_telefone_funcionario foreign key references Pessoa.funcionario(id)
)
go
...
create table Transacao.venda(
	...
	idresponsavel int not null constraint fk_venda_responsavel foreign key references Administracao.responsavel(id),
	dtVenda not null date constraint df_dtVenda_venda default(getdate())
)
go

create table Transacao.compra(
	id int constraint pk_compra primary key identity(1,1),
	idresponsavel int not null constraint fk_compra_responsavel references Administracao.responsavel(id),
	dtCompra date not null constraint df_dtCompra_compra default(getdate())
)
go

create table Transcao.pagamento(
	...
	dtPagamento date not null constraint df_dtPagamento_pagamento defautl(getdate()),
